...
import demo.model.ElasticSearchModel;
import demo.util.ElasticSearchUtil;
import demo.util.EsPageUtil;
...
/**
 *https://blog.csdn.net/li521wang/article/details/83792552
 *
...
public class EmployeeController {
     ...
     * 测试索引
     */
    private String indexName = "rent";
    /**
     * 类型
     */
    private String esType = "community";
    ...
    public String queryMatchData() {
        ...
        if (matchPhrase == Boolean.TRUE) {
            //不进行分词搜索
            boolQuery.must(QueryBuilders.matchPhraseQuery("communityname", "m"));
        } else {
            boolQuery.must(QueryBuilders.matchQuery("communityname", "m-m"));
        }
        List<Map<String, Object>> list = ElasticSearchUtil.
                searchListData(indexName, esType, boolQuery, 10, "communityname", null, "communityname");
    ...
    public String queryPage(String startPage, String pageSize) {
        if (StringUtils.isNotBlank(startPage) && StringUtils.isNotBlank(pageSize)) {
            ...
            boolQuery.must(QueryBuilders.rangeQuery("date").from("2018-04-25T08:33:44.840Z")
                    .to("2019-04-25T10:03:08.081Z"));
            EsPageUtil list = ElasticSearchUtil.searchDataPage(indexName, esType, Integer.parseInt(startPage), Integer.parseInt(pageSize), boolQuery, null, null, null);
            return JSONObject.toJSONString(list);
        } else {
